...
namespace FirstPersonMobileTools.DynamicFirstPerson
{
        ...
        private float m_speed
        {
            get
            {
                if (Input_Crouch)
                    ...
                    return m_RunSpeed;
                else if (Input_Movement.magnitude != 0 || External_Input_Movement.magnitude != 0)
                    return m_WalkSpeed;
                else
                    return 0.0f;
            }
    ...
    public Vector2 External_Input_Movement;
        ...
        private void Handle_Crouch()
        {
            ...
            {
                // Check if there is enough space above to stand up
                Vector3 Origin = transform.position + (transform.localScale.y / m_OriginalScale.y) * m_CharacterController.height * Vector3.up;
                float distanceToCheck = m_OriginalScale.y - transform.localScale.y + 0.2f;
                
                if (!Physics.Raycast(Origin, Vector3.up, distanceToCheck)) 
                {
            ...
            void CrouchTransition(float TargetHeight, float value)
            {
                ...
                // Origin is on top of head to avoid any collision with the player itself
                Vector3 Origin = transform.position + (transform.localScale.y / m_OriginalScale.y) * m_CharacterController.height * Vector3.up;
